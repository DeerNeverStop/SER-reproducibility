"""Generate and validate the frozen N14R2 split and unit plan.

Usage:
  python -m v2_1.n14r2.plan --manifest v2/manifests/cremad_manifest.csv --out <directory>
"""
...
from .spec import (ALL_DRAW_IDS, CELLS, CORPUS_LEVEL, HPO_GRID, MODEL, N_CLASSES,
                   ...
                   STUDY_ID, N_PODS, FIT_WORKERS_PER_POD, pod_index_for_draw,
                   canonical_json, sha256_json, validate_spec)

EST_GPU_SEC_PER_UNIT = 0.0  # Unestimated sentinel, not a zero-cost prediction.
...
def load_manifest(path: Path) -> tuple[list[dict[str, Any]], str, dict[str, Any]]:
    ...
    if len({int(r["label_index"]) for r in kept}) != N_CLASSES:
        raise ValueError("N14R2 requires exactly six label_index levels")
    hygiene = {
        "schema": "ser26-n14r2-hygiene-1",
...
def make_plan(rows: list[dict[str, str]], manifest_sha256: str) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    if set(y.tolist()) != set(range(N_CLASSES)):
        raise ValueError("N14R2 requires all six frozen label indices 0..5")
    ...
    for analysis_order, draw_id in enumerate(ALL_DRAW_IDS):
        ...
        cell_docs = {
            cell: {
                "key": f"n14r2__d{draw_id:02d}__{cell}",
            ...
            for cell in CELLS:
                ...
                for config_index, config in enumerate(HPO_GRID):
                    ...
                    row = {
                        ...
                        "arm": "N14R2", "manifest_sha256": manifest_sha256, "draw_id": draw_id,
                        ...
                        "cap_group": "N14R2", "conditional": 0, "truncation_rank": 99, "status": "pending",
                    }
                    plan.append(row)
...
def validate_plan(plan: list[dict[str, Any]], split_docs: dict[str, dict[str, Any]]) -> None:
    ...
    expected_split_keys = {f"n14r2__d{d:02d}__{c}" for d in ALL_DRAW_IDS for c in CELLS}
    if set(split_docs) != expected_split_keys:
        raise RuntimeError("plan must contain exactly 56 N14R2 split documents")
    split_hashes = {key: sha256_json(doc) for key, doc in split_docs.items()}
    numeric_fields = {
        "draw_id", "analysis_order", "fold", "train_rep", "config_index", "r",
        "seed_index", "split_seed", "inner_seed", "train_seed", "n_fit",
        "n_val", "n_test", "conditional", "truncation_rank",
    }
    for row in plan:
        if set(row) != set(PLAN_FIELDS):
            raise RuntimeError("plan fields differ from the frozen schema")
        d, f, ci = int(row["draw_id"]), int(row["fold"]), int(row["config_index"])
        c = row["cell"]
        if d not in ALL_DRAW_IDS or f not in range(N_FOLDS) or c not in CELLS or ci not in range(8):
            raise RuntimeError("plan unit outside frozen draw/fold/cell/config grid")
        expected_values = {
            "study_id": STUDY_ID, "spec_version": SPEC_VERSION, "arm": STUDY_ID,
            "cap_group": STUDY_ID, "draw_role": _draw_role(d), "analysis_order": d,
            "r": d, "train_rep": 0, "seed_index": 0,
            "split_seed": seeds.split_seed(d), "inner_seed": seeds.inner_seed(d, f, c),
            "train_seed": seeds.train_seed(d, f), "config_sha256": sha256_json(HPO_GRID[ci]),
            "split_sha256": split_hashes[f"n14r2__d{d:02d}__{c}"],
        }
        for name, expected in expected_values.items():
            value = int(row[name]) if name in numeric_fields else row[name]
            if value != expected:
                raise RuntimeError(f"invalid N14R2 plan field: {name}")
        scientific = {
            k: int(row[k]) if k in numeric_fields else row[k]
            for k in PLAN_FIELDS if k not in {"unit_id", "est_gpu_sec", "status"}
        }
        scientific.update(seed_namespace=SPEC["seed_namespace"], config=HPO_GRID[ci])
        if row["unit_id"] != sha256_json(scientific):
            raise RuntimeError("plan unit identity does not match scientific fields")
    for draw_id in ALL_DRAW_IDS:
        ...
        gr = split_docs[f"n14r2__d{draw_id:02d}__GR_hpo"]
        gg = split_docs[f"n14r2__d{draw_id:02d}__GG_hpo"]
...
def build(manifest: Path, out: Path) -> dict[str, Any]:
    ...
    summary = {
        ...
        "nodes": N_PODS, "fit_workers_per_node": FIT_WORKERS_PER_POD,
        "draw_to_node": {str(d): pod_index_for_draw(d) for d in ALL_DRAW_IDS},
        "agent_guard_usd": SPEC["budget"]["agent_guard_usd"],
        "agent_guard_is_scientific_stopping_rule": False,
