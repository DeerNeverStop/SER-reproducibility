"""Frozen, domain-separated seed table for N14R2."""
...
def train_seed(draw_id: int, fold: int, train_rep: int = 0) -> int:
    ...
    _check_draw(draw_id)
    if isinstance(fold, bool) or not isinstance(fold, int) or fold not in range(N_FOLDS) or train_rep != 0:
...
def _check(draw_id: int, fold: int, cell: str) -> None:
    _check_draw(draw_id)
    if isinstance(fold, bool) or not isinstance(fold, int) or fold not in range(N_FOLDS) or cell not in CELLS:
