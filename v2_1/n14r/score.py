"""Independent scorer for the N14R outer-draw rerun.
...
SCHEMA = "ser-v2.1-n14r-results-1"
EXPECTED_DRAWS = 24
N_FOLDS = 5
N_CLASSES = 6
...
def _normal_cell(value: str) -> str:
    ...
    try:
        ...
        raise ScoreError(f"unknown N14R cell {value!r}") from exc
...
def _validate_plan_hygiene(
    ...
    if (hygiene.get("schema") != "ser26-n14r-hygiene-1"
...
def _load_candidate(
    row: dict[str, str], units_dir: Path, splits: SplitStore, manifest: dict[str, dict[str, Any]],
    configs: dict[str, dict[str, Any]],
) -> Candidate:
    unit_id = row["unit_id"]
    paths = _unit_paths(units_dir, unit_id)
...
def _validate_completion_lock(plan_dir: Path, run_dir: Path, rows: list[dict[str, str]]) -> tuple[list[int], dict[str, Any]]:
    completion_path = run_dir / "completion.json"
    if not completion_path.is_file():
        raise ScoreError("completion.json is missing; outcomes must not be parsed before the run is closed")
    completion = _read_json(completion_path)
    if completion.get("schema") != "ser26-n14r-completion-1" or completion.get("study_id") != "N14R":
        raise ScoreError("completion.json study identity mismatch")
    if completion.get("status") != "complete":
        return [], {"completion_status": completion.get("status"), "completion": completion}
    lock_path = run_dir / "analysis_lock.json"
    if not lock_path.is_file():
        raise ScoreError("complete run lacks analysis_lock.json")
    lock = _read_json(lock_path)
    if lock.get("schema") != "ser26-n14r-analysis-lock-1" or lock.get("study_id") != "N14R":
    ...
    if pins.get("schema") != "ser26-n14r-pins-1" or pins.get("content_manifest_sha256") != _canonical_sha256(pins_payload):
    ...
    if not ledger_path.is_file() or lock.get("execution_ledger_sha256") != _sha256(ledger_path):
        raise ScoreError("analysis lock execution ledger hash mismatch")
    environment_path = run_dir / "execution_environment.json"
    if not environment_path.is_file():
        raise ScoreError("execution environment record is missing")
    environment = _read_json(environment_path)
    environment_payload = {key: value for key, value in environment.items() if key not in {"schema", "fingerprint_sha256"}}
    if (environment.get("fingerprint_sha256") != lock.get("environment_fingerprint")
            or environment.get("fingerprint_sha256") != _canonical_sha256(environment_payload)):
        raise ScoreError("analysis lock execution-environment mismatch")
    if environment.get("plan_files") != expected_plan_hashes or environment.get("preregistration_files") != preregistration_hashes:
        raise ScoreError("execution environment does not commit the locked plan/preregistration")
    pinned_external = {
        entry.get("name"): (entry.get("sha256"), int(entry.get("size_bytes", -1)))
        for entry in pins.get("external_feature_cache", [])
    }
    recorded_external = {
        entry.get("basename"): (entry.get("sha256"), int(entry.get("size_bytes", -1)))
        for entry in environment.get("external_feature_cache", [])
    }
    if not pinned_external or recorded_external != pinned_external:
        raise ScoreError("execution environment feature-cache record differs from PINS.json")
    ...
    for row in selected:
        ...
        if not unit_dir.is_dir() or artifacts[row["unit_id"]] != _artifact_hashes(unit_dir):
            raise ScoreError(f"analysis lock artifact hash mismatch for {row['unit_id']}")
    audit_path = run_dir / "n14r_execution_ledger.jsonl"
    if not audit_path.is_file():
        raise ScoreError("N14R audit ledger is missing")
    audit_bytes = audit_path.read_bytes()
    prefix = bytearray()
    found_lock_event = False
    events_before_lock: list[dict[str, Any]] = []
    for line in audit_bytes.splitlines(keepends=True):
        try:
            event = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ScoreError("malformed N14R audit ledger") from exc
        if event.get("event") == "analysis_lock":
            found_lock_event = True
            if event.get("analysis_lock_sha256") != _sha256(lock_path):
                raise ScoreError("audit ledger analysis-lock file hash mismatch")
            break
        events_before_lock.append(event)
        prefix.extend(line)
    if not found_lock_event or hashlib.sha256(bytes(prefix)).hexdigest() != lock.get("audit_ledger_prefix_sha256"):
        raise ScoreError("analysis lock audit-ledger prefix mismatch")
    void_ids = {int(value) for value in (completion.get("void_draws") or {})}
    audit_void_ids = {int(event["draw_id"]) for event in events_before_lock if event.get("event") == "draw_void"}
    if audit_void_ids != void_ids:
        raise ScoreError("completion void draws disagree with the audit ledger")
    expected_ids = _expected_analysis_draw_ids(void_ids)
    if draw_ids != expected_ids:
        raise ScoreError("analysis lock is not the first 24 non-void draws")
    attempted_reserves = sorted({draw for draw in (*draw_ids, *void_ids) if draw >= 24})
    if attempted_reserves and attempted_reserves != list(range(24, max(attempted_reserves) + 1)):
        raise ScoreError("reserve draws were not attempted in ascending contiguous order")
    for reserve in attempted_reserves:
        activation_index = next((i for i, event in enumerate(events_before_lock)
                                 if event.get("event") == "draw_activated" and int(event.get("draw_id", -1)) == reserve), None)
        if activation_index is None:
            raise ScoreError(f"reserve draw {reserve} lacks a draw_activated audit event")
        prior_voids = {int(event["draw_id"]) for event in events_before_lock[:activation_index]
                       if event.get("event") == "draw_void"}
        if len(prior_voids) < reserve - 23:
            raise ScoreError(f"reserve draw {reserve} was activated before enough earlier draws were void")
    return draw_ids, {
    ...
    *,
    ...
    for row in rows:
        try:
            ...
            if (row["study_id"] != "N14R" or row["spec_version"] != "1.0.0" or row["arm"] != "N14R"
            ...
            scientific["seed_namespace"] = "SER26-N14R"
    ...
    locked_draw_ids, source_hashes = _validate_completion_lock(plan_dir, run_dir, rows)
    if not locked_draw_ids:
        reason = f"run completion status is {source_hashes.get('completion_status')!r}; N14R is not tested"
        ...
        result = {
            ...
            "D09R": {"descriptive_only": True, "n": 0, "values": [], "mean": None, "sd": None,
    ...
    for draw in sorted(by_draw, key=lambda d: (draw_order.get(d, 10**9), d)):
        ...
        for key in sorted(episodes):
            ...
            for row in episode_rows:
                try:
                    candidates.append(_load_candidate(row, units_dir, splits, manifest, configs))
    ...
    if enough:
        ...
        reason = f"only {len(selected_complete)}/{EXPECTED_DRAWS} complete draws; N14R is not tested"
...
def _default_output(run_dir: Path) -> Path:
    if run_dir.name == "n14r" and run_dir.parent.name == "runs":
        return run_dir.parent.parent / "results" / "n14r_results.json"
    return run_dir / "n14r_results.json"
