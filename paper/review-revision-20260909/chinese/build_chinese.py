...
def inline(text: str) -> str:
    ...
    def linked(match):
        ...
        if parsed.scheme in ("http", "https"):
            url = target
        elif not parsed.scheme and not parsed.netloc and LINK_SOURCE is not None:
            local = (LINK_SOURCE / target).resolve()
            require(local.is_relative_to(LINK_REPO), "link target escapes repository")
            url = "https://github.com/DeerNeverStop/SER-reproducibility/blob/v2026.09.09/" + quote(local.relative_to(LINK_REPO).as_posix())
        else:
            raise ValueError("unsupported Markdown link")
        return f'<link href="{html.escape(url, quote=True)}" color="#126F79">{label}</link>'
    text = re.sub(
        r"\[([^\]]+)\]\(([^)]+)\)",
        linked,
        text,
...
class CompanionDoc(BaseDocTemplate):
    def __init__(self, destination, *, title="论文中文解释版"):
    ...
    def decorate(self, canvas, doc):
        ...
        canvas.setFont("CN", 8)
        canvas.drawString(MARGIN, PAGE_H - 32, "SER  |  论文中文解释版")
        canvas.drawRightString(PAGE_W - MARGIN, PAGE_H - 32, "Tian Xie  ·  2026-09-09")
        canvas.setStrokeColor(RULE)
        canvas.setLineWidth(0.5)
        canvas.line(MARGIN, PAGE_H - 40, PAGE_W - MARGIN, PAGE_H - 40)
        canvas.line(MARGIN, 38, PAGE_W - MARGIN, 38)
        canvas.drawString(MARGIN, 24, "对应 2026-09-09 英文稿  |  方法、结果与结论边界")
...
def markdown_table(lines: list[str], st: dict) -> Table:
    ...
    n = len(cells[0])
    if n == 9:
        widths = [67, 48, 30, *([(TEXT_W - 145) / 6] * 6)]
    elif n == 8:
        widths = [67, 48, 30, *([(TEXT_W - 145) / 5] * 5)]
    elif n == 7:
        widths = [69, 57, 60, 96, 68, 68, TEXT_W - 418]
    elif n == 6:
        widths = [75, 62, 60, 124, 83, TEXT_W - 404]
    elif n == 5:
    ...
    for rownum, row in enumerate(cells):
        ...
        for colnum, content in enumerate(row):
            ...
            if n == 6 and colnum == 1 and content.startswith("窗口变化"):
                value = "窗口变化<br/>" + inline(content[4:])
    ...
    table.setStyle(TableStyle([
        ...
        ("LEFTPADDING", (0, 0), (-1, -1), 4 if n >= 6 else 7),
        ("RIGHTPADDING", (0, 0), (-1, -1), 4 if n >= 6 else 7),
...
def build(source: Path, destination: Path, font_dir: Path,
    ...
    global LINK_SOURCE, LINK_REPO
    source, destination, font_dir = source.resolve(), destination.resolve(), font_dir.resolve()
    LINK_SOURCE, LINK_REPO = source.parent, source.parents[3]
    ...
    while i < len(lines):
        ...
        if line.startswith("!["):
            ...
            story.append(Paragraph(inline(line[2:]), st["intro"]))
    ...
    def portable(path):
        ...
        if value.is_relative_to(repo):
            return value.relative_to(repo).as_posix()
        if value.parent == font_dir:
            return "external-fonts/" + value.name
        return "external-build/" + value.name
    receipt = dict(schema="ser-chinese-companion-build-20260909-1",
        created_at=datetime.now(timezone.utc).isoformat(), inputs_sha256={portable(k):v for k,v in pins.items()},
        document_metadata=dict(title=document_title, author="Tian Xie", language="zh-CN"),
        figure_manifest=portable(figure_manifest) if figure_manifest and assets else None,
        figures=[dict(path=portable(p["asset"]["path"]), sha256=p["asset"]["sha256"],
            bytes=len(p["asset"]["data"]), source_cropbox=list(p["asset"]["box"]),
            **{k:v for k,v in p.items() if k != "asset"}) for p in placements],
        output=dict(path=portable(destination), bytes=len(pdf_bytes), sha256=byte_sha(pdf_bytes)),
